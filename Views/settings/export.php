...
<div class="container col-xs-12 col-sm-12 col-md-10 m-auto">
    <div class="card p-3 ">
        ...
        <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" class="text-center p-3">
            <button type="submit" name="export" class="btn success-color-dark text-white float-right"> <?=$op->lang("create backup for data");?> </button>
            <br class="mt-3">
        </form>
        <?php
        ...
        if (count($outputFileNames) > 0) :
            ...
            for ($i = 0; $i < count($outputFileNames); $i++) :
                echo "<li class='list-group-item  rounded-0'>$outputFileNames[$i] <span class='badge badge-primary badge-pill float-left text-white rounded-0 py-2 px-3'> <a href=" . $op->siteSetting("siteUrl") . "filesdir/" . $outputFileNames[$i] . " class='text-white '> ". $op->lang("download")." </a> </span>   </li>";
            endfor;
            echo "</div> </ul>";
        else :
